...
/**
 * 💡 Central API Fetch Engine
 */
async function callApi(action, payload = {}, method = 'POST') {
  try {
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${window.AppState ? window.AppState.authToken || '' : ''}`
    };
    ...
    if (method === 'GET') {
      ...
      options.body = JSON.stringify({
        action: action,
        role: window.AppState ? window.AppState.currentUserRole : '',
        authToken: window.AppState ? window.AppState.authToken : '',
        ...payload
      });
    }
